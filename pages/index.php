<!doctype html>
<html lang="en">

<head>
    <!-- Required meta tags -->
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">

    <!-- Bootstrap CSS -->
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/css/bootstrap.min.css" rel="stylesheet"
        integrity="sha384-EVSTQN3/azprG1Anm3QDgpJLIm9Nao0Yz1ztcQTwFspd3yD65VohhpuuCOmLASjC" crossorigin="anonymous">
    <link rel="stylesheet" href="../css/stylehome.css">

    <title>KING AUTOMOTIVE</title>
    <link rel="stylesheet" href="../css/style.css">
</head>

<body>


    <nav class="navbar navbar-expand-lg navbar-light navbar-dark bg-dark">
        <div class="container-fluid">
            <div class="service-icon">
                <svg xmlns="http://www.w3.org/2000/svg" width="50" height="50" viewBox="0 0 24 24">
                    <path
                        d="M2.827 8.25l.257-.018.076-.005c1.311 0 2.963 4.604 3.343 6.724.63-.22 1.272-.401 1.923-.545-.219-1.122-.637-2.472-1.06-3.562l.172.002c1.549 0 3.271-.571 4.464-2.016 1.191 1.444 2.914 2.016 4.463 2.016l.171-.002c-.424 1.09-.842 2.44-1.058 3.562.649.144 1.291.325 1.921.544.379-2.121 2.031-6.724 3.341-6.724l.076.005.256.018c.986.001 1.828-.803 1.828-1.832 0-1.013-.82-1.833-1.832-1.833-1.217 0-2.104 1.167-1.764 2.329.287.981-1.247 1.935-2.939 1.935-1.613 0-3.37-.867-3.83-3.405-.24-1.324.02-1.671.666-2.321.328-.329.533-.783.533-1.287 0-1.014-.82-1.835-1.834-1.835-1.012 0-1.833.821-1.833 1.833 0 .503.204.958.534 1.287.647.651.907.998.668 2.321-.461 2.539-2.218 3.406-3.831 3.406-1.692 0-3.227-.955-2.94-1.935.34-1.162-.549-2.329-1.764-2.329-1.012 0-1.834.821-1.834 1.834 0 1.029.842 1.833 1.827 1.833zm.033 9.735l2.343 3.335c-.89.769-1.664 1.668-2.277 2.68l-.566-3h-2.36c.807-1.167 1.771-2.136 2.86-3.015zm18.28 0l-2.343 3.335c.89.769 1.664 1.668 2.277 2.68l.566-3h2.36c-.807-1.167-1.771-2.136-2.86-3.015zm-1.383.228l-2.334 3.322c-1.603-.924-3.448-1.464-5.423-1.473-1.975.009-3.82.549-5.423 1.473l-2.334-3.322c2.266-1.386 4.912-2.202 7.757-2.211 2.845.009 5.491.825 7.757 2.211zm-10.908.561l.53-.511-.729-.101-.323-.662-.322.663-.729.101.531.511-.13.725.65-.348.65.348-.128-.726zm3.672-.5l.53-.511-.729-.101-.322-.662-.322.663-.729.101.531.511-.13.725.65-.348.65.348-.129-.726zm3.718.5l.53-.511-.729-.101-.322-.662-.322.663-.729.101.531.511-.131.725.65-.348.65.348-.128-.726z" />
                </svg>
                <span class="text-white"> KING <strong>AUTOMOTIVE</strong></span>
            </div>
            <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarScroll"
                aria-controls="navbarScroll" aria-expanded="false" aria-label="Toggle navigation">
                <span class="navbar-toggler-icon"></span>
            </button>
            <div class="collapse navbar-collapse" id="navbarScroll">
                <ul class="navbar-nav ms-auto">
                    <li class="nav-item">
                        <a class="nav-link active" aria-current="page" href="index.php">Beranda </a>
                    </li>

                    <li class="nav-item">
                        <a class="nav-link" href="#contact">Contact</a>
                    </li>

                    <li class="nav-item">
                        <a class="nav-link" href="register.php">Daftar</a>
                    </li>

                    <li class="nav-item">
                        <a class="nav-link" href="login.php">Masuk</a>
                </ul>
                <form class="d-flex">
                    <input class="form-control me-2" type="search" placeholder="Search" aria-label="Search">
                    <button class="btn btn-outline-success" type="submit">Search</button>
                </form>
            </div>
        </div>
    </nav>

    <!-- <div class="container">
        <div id="carouselExampleControls" class="carousel slide mt-5" data-bs-ride="carousel">
            <div class="carousel-inner">
                <div class="carousel-item active">
                    <img src="../assets/gmb1.png" class="d-block w-100" alt="">
                </div>
                <div class="carousel-item">
                    <img src="../assets/gmb1.png" class="d-block w-100" alt="...">
                </div>
                <button class="carousel-control-prev" type="button" data-bs-target="#carouselExampleControls"
                    data-bs-slide="prev">
                    <span class="carousel-control-prev-icon" aria-hidden="true"></span>
                    <span class="visually-hidden">Previous</span>
                </button>
                <button class="carousel-control-next" type="button" data-bs-target="#carouselExampleControls"
                    data-bs-slide="next">
                    <span class="carousel-control-next-icon" aria-hidden="true"></span>
                    <span class="visually-hidden">Next</span>
                </button>
            </div>
        </div>
        </div> -->


    <div class="container-fluid banner">
        <div class="container text-center">
            <h4 class="display-6">Welcome to</h4>
            <h3 class="display-1">KING AUTOMOTIVE!</h3>
            <a href="Join">
                <button type="button" class="btn btn-danger btn-lg">
                    JOIN WITH US!
                </button>
            </a>
        </div>
    </div>


    <!-- Kategori -->
    <div class="container mt-5">
        <div class="judul-Produk" style="background-color:  #3f3939;  padding: 5px 10px;">
            <h5 class="text-center" style="margin-top: 5px;">KATEGORI</h5>
        </div>


        <div class="row text-center row-container mt-2">
            <div class="col-lg-2 col-md-3 col-sm-4 col-6 mt-5">
                <div class="menu-kategori">
                    <a href="#"><img src="../assets/pelak2.jpeg" class="img-kategori mt-3"></a>
                    <p class="mt-2">VELG</p>
                    <div class="card-body">
                        <a href="singleproduct.php" target="_blank" class="btn btn-primary">View more</a>
                    </div>
                </div>
            </div>



            <div class="col-lg-2 col-md-3 col-sm-4 col-6 mt-5">
                <div class="menu-kategori">
                    <a href="#"><img src="../assets/sb.jpeg" class="img-kategori mt-3"></a>
                    <p class="mt-2">shockbreake</p>
                    <div class="card-body">
                        <a href="singleproduct2.php" target="_blank" class="btn btn-primary">View more</a>
                    </div>
                </div>
            </div>



            <div class="col-lg-2 col-md-3 col-sm-4 col-6 mt-5">
                <div class="menu-kategori">
                    <a href="#"><img src="../assets/master rem.png_.webp" class="img-kategori mt-3"></a>
                    <p class="mt-2">Master Rem</p>
                    <div class="card-body">
                        <a href="singleproduct3.php" target="_blank" class="btn btn-primary">View more</a>
                    </div>
                </div>
            </div>



            <div class="col-lg-2 col-md-3 col-sm-4 col-6 mt-5">
                <div class="menu-kategori">
                    <a href="#"><img src="../assets/kanalpot.jpg" class="img-kategori mt-3"></a>
                    <p class="mt-2">kanalpot</p>
                    <div class="card-body">
                        <a href="singleproduct4.php" target="_blank" class="btn btn-primary">View more</a>
                    </div>
                </div>
            </div>



            <div class="col-lg-2 col-md-3 col-sm-4 col-6 mt-5">
                <div class="menu-kategori">
                    <a href="#"><img src="../assets/Underbone.jpg" class="img-kategori mt-3"></a>
                    <p class="mt-2">Underbone</p>
                    <div class="card-body">
                        <a href="singleproduct5.php" target="_blank" class="btn btn-primary">View more</a>
                    </div>
                </div>
            </div>



            <div class="col-lg-2 col-md-3 col-sm-4 col-6 mt-5">
                <div class="menu-kategori">
                    <a href="#"><img src="../assets/ban2.jpg" class="img-kategori mt-3"></a>
                    <p class="mt-2">BAN</p>
                    <div class="card-body">
                        <a href="singleproduct6.php" target="_blank" class="btn btn-primary">View more</a>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <!-- Kategori -->

    <!-- Produk -->
    <div class="container mt-5">
        <div class="judul-Produk" style="background-color:  #3f3939;  padding: 5px 10px;">
            <h5 class="text-center" style="margin-top: 5px;">PRODUK</h5>
        </div>



        <div class="row">
            <div class="col-lg-2 col-md-2 col-sm-4 col-6 mt-2">
                <div class="Card text-center">
                    <img src="../assets/rcb-logo.webp" class="card-img-top" alt="...">
                    <div class="card-body">
                        <h6 class="card-title">RCB</h6>
                        <div class="btn-produk mt-1">
                            <a href="https://rcb.com/products/" class="btn btn-warning bt-lg"
                                style="font-size: 15px">view more</a>
                        </div>
                    </div>
                </div>
            </div>



            <div class="col-lg-2 col-md-2 col-sm-4 col-6 mt-2">
                <div class="Card text-center">
                    <img src="../assets/Ohlins-Logo.jpg" class="card-img-top" alt="...">
                    <div class="card-body">
                        <h6 class="card-title">OHLINS</h6>
                        <div class="btn-produk mt-1">
                            <a href="https://ohlinsindonesia.com/products/motorcycle/" class="btn btn-warning bt-lg"
                                style="font-size: 15px">view more</a>
                        </div>
                    </div>
                </div>
            </div>



            <div class="col-lg-2 col-md-2 col-sm-4 col-6 mt-2">
                <div class="Card text-center">
                    <img src="../assets/ktm logo.png" class="card-img-top" alt="...">
                    <div class="card-body">
                        <h6 class="card-title">KTM</h6>
                        <div class="btn-produk mt-1">
                            <a href="https://www.ktm.com/en-us/parts---wear/powerwear/products.html?c1=0"
                                class="btn btn-warning bt-lg" style="font-size: 15px">view more</a>
                        </div>
                    </div>
                </div>
            </div>



            <div class="col-lg-2 col-md-2 col-sm-4 col-6 mt-2">
                <div class="Card text-center">
                    <img src="../assets/LOGO KTC.png" class="card-img-top" alt="...">
                    <div class="card-body">
                        <h6 class="card-title">KTC</h6>
                        <div class="btn-produk mt-1">
                            <a href="https://ktc-racing.com/products/controlparts/" class="btn btn-warning bt-lg"
                                style="font-size: 15px">view more</a>
                        </div>
                    </div>
                </div>
            </div>



            <div class="col-lg-2 col-md-2 col-sm-4 col-6 mt-2">
                <div class="Card text-center">
                    <img src="../assets/LOGO FDR.png" class="card-img-top" alt="...">
                    <div class="card-body">
                        <h6 class="card-title">FDR</h6>
                        <div class="btn-produk mt-1">
                            <a href="https://fdrtire.com/product/search?search_by=function&tag=onroad"
                                class="btn btn-warning bt-lg" style="font-size: 15px">view more</a>
                        </div>
                    </div>
                </div>
            </div>



            <div class="col-lg-2 col-md-2 col-sm-4 col-6 mt-2">
                <div class="Card text-center">
                    <img src="../assets/logo TDR.jpg" class="card-img-top" alt="...">
                    <div class="card-body">
                        <h6 class="card-title">TDR</h6>
                        <div class="btn-produk mt-1">
                            <a href="https://tdr-racing.com/products-2/handling/suspension"
                                class="btn btn-warning bt-lg" style="font-size: 15px">view more</a>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <!-- Produk -->

    <!-- contact -->
    <div class="mt-5 d-flex justify-content-evenly" id="contact">
        <!-- Contact Item - 1 -->
        <div class="contact-item-icon">
            <a href="https://maps.app.goo.gl/1YLbdjshzC4rEte97?g_st=ic">
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                    <path
                        d="M12 1c-3.148 0-6 2.553-6 5.702 0 3.148 2.602 6.907 6 12.298 3.398-5.391 6-9.15 6-12.298 0-3.149-2.851-5.702-6-5.702zm0 8c-1.105 0-2-.895-2-2s.895-2 2-2 2 .895 2 2-.895 2-2 2zm12 14h-24l4-8h3.135c.385.641.798 1.309 1.232 2h-3.131l-2 4h17.527l-2-4h-3.131c.435-.691.848-1.359 1.232-2h3.136l4 8z" />
                </svg>
            </a>
        </div>
        <div class="contact-item-detail">
            <h5>Address</h5>
            <p>Gegerkalong, Sukasari, Gg Haji Ridho 1 49 A </p>
        </div>

        <!-- Contact Iten - 2 -->

        <div class="contact-item-icon">
            <a href="https://wa.me/6282262864260">
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                    <path
                        d="M20 22.621l-3.521-6.795c-.008.004-1.974.97-2.064 1.011-2.24 1.086-6.799-7.82-4.609-8.994l2.083-1.026-3.493-6.817-2.106 1.039c-7.202 3.755 4.233 25.982 11.6 22.615.121-.055 2.102-1.029 2.11-1.033z" />
                </svg>
            </a>

        </div> 
        <div class="contact-item-detail">
            <h5>Phone</h5>
            <p>+62 822-6286-4260</p>
        </div>

        <!-- Contact Item - 3 -->

        <div class="contact-item-icon">
            <a href="">
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                    <path
                        d="M0 3v18h24v-18h-24zm21.518 2l-9.518 7.713-9.518-7.713h19.036zm-19.518 14v-11.817l10 8.104 10-8.104v11.817h-20z" />
                </svg>
            </a>

        </div>
        <div class="contact">
            <h5>Email</h5>
            <p>Kingsport@gmail.com</p>
        </div>

    </div>
    </div>
    </div>
    </div>
    </section>
    <!-- contact -->

    <!-- footer -->
    <footer class="bg-light p-5 mt-5">
        <div class="container">
            <div class="row">
                <div class="col">
                    <svg xmlns="http://www.w3.org/2000/svg" width="50" height="50" viewBox="0 0 24 24">
                        <path
                            d="M2.827 8.25l.257-.018.076-.005c1.311 0 2.963 4.604 3.343 6.724.63-.22 1.272-.401 1.923-.545-.219-1.122-.637-2.472-1.06-3.562l.172.002c1.549 0 3.271-.571 4.464-2.016 1.191 1.444 2.914 2.016 4.463 2.016l.171-.002c-.424 1.09-.842 2.44-1.058 3.562.649.144 1.291.325 1.921.544.379-2.121 2.031-6.724 3.341-6.724l.076.005.256.018c.986.001 1.828-.803 1.828-1.832 0-1.013-.82-1.833-1.832-1.833-1.217 0-2.104 1.167-1.764 2.329.287.981-1.247 1.935-2.939 1.935-1.613 0-3.37-.867-3.83-3.405-.24-1.324.02-1.671.666-2.321.328-.329.533-.783.533-1.287 0-1.014-.82-1.835-1.834-1.835-1.012 0-1.833.821-1.833 1.833 0 .503.204.958.534 1.287.647.651.907.998.668 2.321-.461 2.539-2.218 3.406-3.831 3.406-1.692 0-3.227-.955-2.94-1.935.34-1.162-.549-2.329-1.764-2.329-1.012 0-1.834.821-1.834 1.834 0 1.029.842 1.833 1.827 1.833zm.033 9.735l2.343 3.335c-.89.769-1.664 1.668-2.277 2.68l-.566-3h-2.36c.807-1.167 1.771-2.136 2.86-3.015zm18.28 0l-2.343 3.335c.89.769 1.664 1.668 2.277 2.68l.566-3h2.36c-.807-1.167-1.771-2.136-2.86-3.015zm-1.383.228l-2.334 3.322c-1.603-.924-3.448-1.464-5.423-1.473-1.975.009-3.82.549-5.423 1.473l-2.334-3.322c2.266-1.386 4.912-2.202 7.757-2.211 2.845.009 5.491.825 7.757 2.211zm-10.908.561l.53-.511-.729-.101-.323-.662-.322.663-.729.101.531.511-.13.725.65-.348.65.348-.128-.726zm3.672-.5l.53-.511-.729-.101-.322-.662-.322.663-.729.101.531.511-.13.725.65-.348.65.348-.129-.726zm3.718.5l.53-.511-.729-.101-.322-.662-.322.663-.729.101.531.511-.131.725.65-.348.65.348-.128-.726z" />
                    </svg>
                    <span>copyright @2024 | Created and Develope by <a href="https://www.instagram.com/jojeyyy_/"
                            class="text-decoration-none text-dark fw-bold">JMATULESSY</a>
                        <span class="ms-5">
                            <a href="https://www.facebook.com/jhosua.jeremi"><svg xmlns="http://www.w3.org/2000/svg"
                                    width="24" height="24" viewBox="0 0 24 24">
                                    <path
                                        d="M12 0c-6.627 0-12 5.373-12 12s5.373 12 12 12 12-5.373 12-12-5.373-12-12-12zm3 8h-1.35c-.538 0-.65.221-.65.778v1.222h2l-.209 2h-1.791v7h-3v-7h-2v-2h2v-2.308c0-1.769.931-2.692 3.029-2.692h1.971v3z" />
                                </svg></a>
                            <a href="https://wa.me/6282262864260"><svg xmlns="http://www.w3.org/2000/svg" width="24"
                                    height="24" viewBox="0 0 24 24">
                                    <path
                                        d="M12 2c5.514 0 10 4.486 10 10s-4.486 10-10 10-10-4.486-10-10 4.486-10 10-10zm0-2c-6.627 0-12 5.373-12 12s5.373 12 12 12 12-5.373 12-12-5.373-12-12-12zm-5.971 18l.844-3.081c-.52-.902-.794-1.925-.794-2.973.001-3.279 2.668-5.946 5.946-5.946 1.591.001 3.084.62 4.206 1.744 1.123 1.124 1.741 2.618 1.74 4.207-.001 3.279-2.669 5.946-5.946 5.946-.995 0-1.976-.25-2.844-.724l-3.152.827zm3.298-1.903c.838.497 1.638.796 2.696.796 2.724 0 4.943-2.217 4.944-4.942.001-2.731-2.208-4.945-4.94-4.946-2.726 0-4.943 2.217-4.944 4.942 0 1.112.326 1.946.873 2.817l-.499 1.824 1.87-.491zm5.694-2.732c-.037-.062-.136-.099-.285-.174-.149-.074-.879-.434-1.015-.483-.136-.05-.235-.074-.334.074-.099.149-.384.483-.47.583-.087.099-.173.112-.322.037-.149-.074-.627-.231-1.195-.737-.442-.394-.74-.881-.827-1.029-.087-.149-.009-.229.065-.303l.223-.26.149-.248c.05-.099.025-.186-.012-.26l-.458-1.103c-.121-.29-.243-.25-.334-.255l-.286-.007c-.099 0-.26.037-.396.186s-.52.508-.52 1.24c0 .731.532 1.438.607 1.537.074.099 1.048 1.6 2.538 2.243.354.153.631.245.847.313.356.113.68.097.936.059.285-.043.879-.359 1.003-.706.123-.348.123-.645.086-.707z" />
                                </svg></a>
                        </span>


                    </span>
                </div>
            </div>
        </div>
    </footer>
    <!-- footer -->
    <!-- Optional JavaScript; choose one of the two! -->

    <!-- Option 1: Bootstrap Bundle with Popper -->
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/js/bootstrap.bundle.min.js"
        integrity="sha384-MrcW6ZMFYlzcLA8Nl+NtUVF0sA7MsXsP1UyJoMp4YLEuNSfAP+JcXn/tWtIaxVXM"
        crossorigin="anonymous"></script>

    <!-- Option 2: Separate Popper and Bootstrap JS -->
    <!--
    <script src="https://cdn.jsdelivr.net/npm/@popperjs/core@2.9.2/dist/umd/popper.min.js" integrity="sha384-IQsoLXl5PILFhosVNubq5LC7Qb9DXgDA9i+tQ8Zj3iwWAwPtgFTxbJ8NT4GN1R8p" crossorigin="anonymous"></script>
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/js/bootstrap.min.js" integrity="sha384-cVKIPhGWiC2Al4u+LWgxfKTRIcfu0JTxR+EQDz/bgldoEyl4H0zUF0QKbrJ0EcQF" crossorigin="anonymous"></script>
    -->
</body>

</html>